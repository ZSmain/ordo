<script lang="ts">
	...
	// Load sessions when drawer opens with a new activity
	$effect(() => {
		if (open && activity) {
			loadSessions();
		}
	...
	async function loadSessions() {
		...
		error = null;
		try {
			sessions = await getSessionsForActivity({
				userId,
	...
	let totalDuration = $derived(sessions.reduce((sum, s) => sum + (s.duration || 0), 0));
